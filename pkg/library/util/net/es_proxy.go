...
// Next Stream from source es instance. "done" is an indicator that the stream is
// over
...
READ_DOCS:
	for {
		...
		for _, key := range []string{"_index", "_type", "_source", "_id"} {
			if _, ok := docI[key]; !ok {
				fmt.Printf("failed parsing document: %v", docI)
				break READ_DOCS
			}
		}
...
// NewScroll make the initial scroll req
...
// BulkPost Post to es as bulk and reset the data buffer
